...
#                                     Icono" (el de Scriptya o el de un
#                                     script instalado) o "Ver Historial"
#                                     (últimas ejecuciones).
...
# set_desktop_icon <fichero.desktop> <icono> -> sustituye solo la línea
# Icon= de un .desktop ya existente, sin tocar Name/Exec/Comment. La usa
# change_icon_for_script para actualizar a la vez la copia del menú y
# la del Escritorio de un mismo script instalado.
set_desktop_icon() {
    local file="$1" icon="$2" tmp
    [[ -f "$file" ]] || return 1
    tmp="$(mktemp)" || return 1
    grep -v '^Icon=' "$file" > "$tmp" 2>/dev/null
    printf 'Icon=%s\n' "$icon" >> "$tmp"
    mv -f "$tmp" "$file"
}
...
# choose_and_process_icon <script|""> <id> -> asistente interactivo para
# elegir la imagen del icono (metadatos ICON:, navegando, ruta escrita o
# icono del tema) y procesarla igual que en la instalación. Deja el
# resultado (ruta de fichero final o nombre de icono del tema) en
# PICKED_ICON_FINAL. La usan install_icon_for_script y
# change_icon_for_script, para no duplicar este bloque.
choose_and_process_icon() {
    local script="$1" id="$2"
    PICKED_ICON_FINAL="utilities-terminal"
...
change_icon_for_script() {
    ...
    choose_and_process_icon "$REG_SOURCE" "$id"
    local icon_final="$PICKED_ICON_FINAL"
...
change_main_icon() {
    ...
    choose_and_process_icon "" "_scriptya_self"
    local icon_final="$PICKED_ICON_FINAL"
...
# menú raíz. A diferencia de pick_installed_icon_and_change (que solo
# lista scripts), aquí se puede elegir también el icono principal de
# Scriptya, para cubrir cualquier icono de acceso directo desde un
# único sitio.
...
Uso:
  scriptya.sh              Abre el menú de scripts. Al final del
                                  ...
                                  "Cambiar Icono" (el de Scriptya o el
                                  de un script instalado).
