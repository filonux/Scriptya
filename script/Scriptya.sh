...
install_icon_for_script() {
    ...
    # Si el valor es una ruta relativa a un fichero de imagen (típicamente
    # venida de "# ICON: icono.png" en los metadatos, o escrita a mano),
    # se interpreta relativa a la CARPETA DEL SCRIPT, no al directorio
    # desde el que se haya lanzado scriptya (que casi nunca coincide
    # y antes hacía que el icono se guardase mal, como si fuera un nombre
    # de icono de tema, sin avisar de nada).
    if [[ -n "$icon_value" && "$icon_value" != /* && "$icon_value" != "~"* ]]; then
        case "${icon_value,,}" in
    ...
    local icon_final="utilities-terminal"
    if [[ -n "$icon_value" ]]; then
        ...
        if [[ -f "$icon_path" ]]; then
            ...
            # Sufijo único para el NOMBRE del fichero (no para $id, que
            # debe seguir siendo estable para reconocer "el mismo
            # script" en el registro). Es la clave del icono: si cada
            # icono nuevo tiene un nombre distinto, es imposible que
            # Cinnamon/Nemo sirvan una miniatura o pixbuf cacheado de un
            # fichero ANTERIOR con ese mismo nombre. Antes se reutilizaba
            # siempre "${id}.png", así que al desinstalar un script y
            # volver a instalarlo con un icono distinto, el nombre de
            # fichero no cambiaba (aunque el contenido sí) y el
            # escritorio/menú seguían pintando el icono viejo hasta
            # cerrar sesión.
            local icon_tag
            icon_tag="$(unique_icon_tag)"

            if [[ "$ext_lower" == "svg" ]]; then
                # Los SVG son vectoriales (ya "escalan" perfectamente y
                # suelen traer su propia transparencia): se copian tal
                # cual, sin pasarlos por el procesado a 256x256.
                ...
                if [[ -n "$processed" && -f "$processed" ]]; then
                    icon_final="$processed"
                    print_success "Icono ajustado (tamaño y transparencia) y copiado a $ICONS_DIR"
                else
                    # Sin ImageMagick, o el procesado falló: se copia el
                    # fichero tal cual, sin ajustar tamaño ni transparencia.
            ...
            # No es un fichero existente: se asume que es el nombre de un
            # icono del tema del sistema (p.ej. "folder" o "utilities-terminal").
            icon_final="$icon_value"
        fi
    fi
