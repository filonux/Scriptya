...
# de la configuración existente. Escribe vía temporal+rename (ver
# atomic_write): una escritura interrumpida no deja CONFIG_FILE truncado
# ni a medio escribir.
set_language() {
    local new_lang="$1" content line found="false"
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        content="$(
            ...
            [[ "$found" == "false" ]] && printf 'LANGUAGE=%q\n' "$LANGUAGE"
        )"
    else
        content="$(printf '# %s\nLANGUAGE=%q' "$(tr "Configuración de Scriptya")" "$LANGUAGE")"
    fi
...
# sweep_stale_temp <dir> <patrón> -> borra restos de un temporal de
# escritura atómica (ver atomic_write) que un kill/OOM/corte de luz
# interrumpió a mitad de escritura, antes de que su propio "rm -f" de
# limpieza llegara a ejecutarse. El margen de 10 min evita tocar el
# temporal de una escritura legítima en curso. La usan todas las
# funciones de este fichero que escriben vía temporal+rename.
sweep_stale_temp() { find "$1" -maxdepth 1 -name "$2" -mmin +10 -delete 2>/dev/null; }

# atomic_write <destino> <contenido> -> escribe <contenido> en <destino> a
# través de un temporal en el MISMO directorio + rename: una escritura
# interrumpida a medias (disco lleno, proceso matado) no deja <destino>
# truncado, y al estar en el mismo filesystem el "mv" final es un rename
# real, no una copia entre dispositivos. Conserva los permisos de
# <destino> si ya existía. Devuelve 1 si algo falla, sin tocar <destino>.
atomic_write() {
    local dest="$1" content="$2" dir tmp
    dir="$(dirname -- "$dest")"
    mkdir -p "$dir" 2>/dev/null
    sweep_stale_temp "$dir" '.scriptya-tmp.*'
    tmp="$(mktemp "$dir/.scriptya-tmp.XXXXXX" 2>/dev/null)" || return 1
    printf '%s\n' "$content" > "$tmp" 2>/dev/null || { rm -f "$tmp" 2>/dev/null; return 1; }
    [[ -f "$dest" ]] && chmod --reference="$dest" "$tmp" 2>/dev/null
    mv -f "$tmp" "$dest" 2>/dev/null || { rm -f "$tmp" 2>/dev/null; return 1; }
}

# write_desktop_file <destino> <contenido> -> crea el directorio padre y
# escribe <contenido> en <destino> (ver atomic_write). Centraliza la
# escritura de los .desktop (--install, --icons, --desktop) para que un
# fallo de permisos no acabe reportándose como instalación correcta sin
# fichero real.
write_desktop_file() { atomic_write "$1" "$2"; }

# copy_file_atomic <origen> <destino> -> copia <origen> a <destino> vía
# temporal+rename (ver atomic_write), para cuando lo que hay que copiar
# es OTRO FICHERO (el propio launcher, un .desktop ajeno) y no un
# string en memoria. Si <origen> ya ES <destino>, no hace nada. Si el
# "cp" se interrumpe o falla, <destino> queda intacto (o sin crear).
copy_file_atomic() {
    local src="$1" dest="$2" dir tmp
    [[ "$src" -ef "$dest" ]] && return 0
    dir="$(dirname -- "$dest")"
    mkdir -p "$dir" 2>/dev/null
    sweep_stale_temp "$dir" '.scriptya-cp.*'
    tmp="$(mktemp "$dir/.scriptya-cp.XXXXXX" 2>/dev/null)" || return 1
...
EOF
    ...
    atomic_write "$dir/actualizar_sistema.sh" "$content" || ok=false
...
EOF
    ...
    atomic_write "$dir/Backup/copia_documentos.sh" "$content" || ok=false
...
do_install() {
    ...
    elif copy_file_atomic "$SCRIPT_PATH" "$INSTALL_DIR/scriptya.sh" && chmod +x "$INSTALL_DIR/scriptya.sh" 2>/dev/null; then
...
registry_write() {
    ...
    local content
    content="$(printf '%s\n%s\n%s\n%s\n%s\n%s' "$2" "$3" "$4" "$5" "$6" "${7:-script}")"
    atomic_write "$REGISTRY_DIR/$1.meta" "$content"
}
...
# final, a lo bruto, pisaría esas u otras secciones. El temporal se crea
# en el mismo directorio que el destino (no en /tmp) para que el "mv -f"
# final sea un rename atómico del mismo filesystem, no una copia entre
# dispositivos que podría dejar el .desktop a medias; si falla, se borra
# el temporal y se devuelve 1 en vez de dejarlo huérfano.
set_desktop_field() {
    local file="$1" field="$2" value="$3" tmp dir line in_entry="false" written="false" ok=true
    [[ -f "$file" ]] || return 1
    dir="$(dirname -- "$file")"
    sweep_stale_temp "$dir" '.scriptya-tmp.*'
    tmp="$(mktemp "$dir/.scriptya-tmp.XXXXXX" 2>/dev/null)" || return 1
    ...
    if [[ "$ok" == "true" ]] && mv -f "$tmp" "$file" 2>/dev/null; then
        return 0
    fi
    rm -f "$tmp" 2>/dev/null
    return 1
...
# instalado o si el procesado falla, sin tocar nada. El temporal se crea
# junto al destino (no en /tmp) para que el "mv -f" final sea un rename
# atómico del mismo filesystem, igual que en set_desktop_field.
process_icon_image() {
    local src="$1" dest="$2"
    ...
    local tmp dir
    dir="$(dirname -- "$dest")"
    sweep_stale_temp "$dir" '.scriptya-icon.*.png'
    tmp="$(mktemp "$dir/.scriptya-icon.XXXXXX.png" 2>/dev/null)" || return 1
    ...
    # Solo si NO hay transparencia real se busca un fondo liso (4
    # esquinas del mismo color) que convertir en transparente.
    local -a extra_args=()
    if [[ "$is_opaque" != "false" ]]; then
        ...
        [[ -n "$c1" && "$c1" == "$c2" && "$c1" == "$c3" && "$c1" == "$c4" ]] && extra_args=(-fuzz 8% -transparent "$c1")
    fi

    # El código de salida de "convert" importa tanto como el tamaño del
    # temporal: un fallo a media escritura (disco lleno...) puede dejar
    # "$tmp" no vacío pero incompleto, y sin comprobar el "$?" se habría
    # aceptado igual como si fuera un PNG válido.
    if convert "$src" "${extra_args[@]}" -resize 256x256 -background none -gravity center \
        -extent 256x256 "$tmp" 2>/dev/null && [[ -s "$tmp" ]]; then
...
change_icon_for_installed_app() {
    ...
    if [[ "$src_file" != "$target" ]] && ! copy_file_atomic "$src_file" "$target"; then
...
# son opcionales. Escribe vía temporal (junto al propio script) +
# rename, y barre restos de una escritura anterior interrumpida antes
# de crear el suyo (ver sweep_stale_temp).
apply_metadata() {
    local script="$1"
    local -a lines=() other_header=() new_meta=()
    local shebang="" i=0 body_start line key m h a html="false" node="false" go="false" lua="false" php="false" php_open="false" dir tmp
    local re_doctype='^[[:space:]]*<!DOCTYPE[[:space:]][^>]*>[[:space:]]*$'
    local re_html_comment='^[[:space:]]*<!--[[:space:]]*.*[[:space:]]*-->[[:space:]]*$'
    local re_js_comment='^[[:space:]]*//[[:space:]]*.*$'
    local re_php_hash_comment='^[[:space:]]*#[[:space:]]*.*$'
    ...
    dir="$(dirname -- "$script")"
    sweep_stale_temp "$dir" '.scriptya-meta.*'
    tmp="$(mktemp "$dir/.scriptya-meta.XXXXXX" 2>/dev/null)" || return 1
    {
        [[ -n "$shebang" ]] && printf '%s\n' "$shebang"
    ...
    chmod --reference="$script" "$tmp" 2>/dev/null
    mv -f "$tmp" "$script" 2>/dev/null || { rm -f "$tmp" 2>/dev/null; return 1; }
}
...
write_nemo_action() {
    local file="$1" name="$2" comment="$3" flag="$4" ext="$5" icon="$6" launcher content
    launcher="$(launcher_exec_path)"
    content="$(
        echo "[Nemo Action]"
        echo "Name=$name"
...
ensure_brand_icon_file() {
    ...
    mkdir -p "$ICONS_DIR" 2>/dev/null
    # Se decodifica a un temporal junto al destino y se renombra: si la
    # decodificación se corta a medias, el PNG roto (pero no vacío) queda
    # en el temporal, no en $BRAND_ICON_FILE, así que la próxima llamada
    # sigue viéndolo "no existe todavía" y reintenta, en vez de darlo por
    # bueno para siempre por el simple hecho de no estar vacío.
    local tmp
    sweep_stale_temp "$ICONS_DIR" '.scriptya-brand.*'
    tmp="$(mktemp "$ICONS_DIR/.scriptya-brand.XXXXXX" 2>/dev/null)" || return 1
    if base64 -d <<< "$SY_BRAND_ICON_B64" > "$tmp" 2>/dev/null && [[ -s "$tmp" ]] && mv -f "$tmp" "$BRAND_ICON_FILE" 2>/dev/null; then
        :
    else
        rm -f "$tmp" 2>/dev/null
    fi
...
do_update() {
    ...
    # copy_file_atomic: si el "cp" se interrumpe o falla a medio
    # escribir, el scriptya.sh instalado queda intacto en su versión
    # anterior en vez de a medias.
    if ! copy_file_atomic "$SCRIPT_PATH" "$INSTALL_DIR/scriptya.sh" || ! chmod +x "$INSTALL_DIR/scriptya.sh" 2>/dev/null; then
